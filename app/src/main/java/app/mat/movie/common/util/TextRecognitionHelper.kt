...
abstract class TextRecognitionHelper {
    abstract fun scanTextFromBitmap(
        bitmap: Bitmap,
        rotation: Float,
        roi: Roi?
    ): Flow<ScanState>
...
class TextRecognitionHelperImpl @Inject constructor() : TextRecognitionHelper() {
    private val textRecognition = TextRecognition.getClient(
    ...
    override fun scanTextFromBitmap(
        ...
        val processedBitmap = preprocessBitmap(
            bitmap,
            rotation,
            roi
        )
        val onCompleteTaskListener = OnCompleteListener<Text> {
            if (it.isSuccessful) {
                ...
                when (val validationResult = ScannedTextValidator.validateText(
                    text
                )) {
                    ...
                    is ScannedTextValidator.ValidationResult.Invalid -> {
                        trySend(
                            ScanState.InvalidText(
                                validationResult.errorResId
                            )
                        )
                    }
                }
        ...
        textRecognition.process(
            ...
            addOnFailureListener(
                onFailureTaskListener
            )
            addOnCanceledListener(
                onCancelTaskListener
            )
        }
